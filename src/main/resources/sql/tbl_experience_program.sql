...
select
    ...
    ep.experience_program_start_date, ep.experience_program_end_date, ep.experience_program_deadline, ep.experience_program_status,
...
from tbl_challenger c join tbl_apply a
on c.apply_id = a.id
join tbl_member m on a.member_id = m.id join tbl_individual_member im on m.id = im.id # view로 만드는 게 좋을듯
join tbl_experience_program ep
on a.experience_program_id = ep.id
join tbl_corp co
on ep.corp_id = co.id
where m.id = 9;
